      ...
      <div className="flex flex-wrap gap-6 justify-center">
        {languages.map(({ _id, name, description }, index) => {
          ...
          return (
            ...
            >
              <Card
                className={`bg-slate-800 rounded-2xl border-2 hover:shadow-2xl hover:scale-[1.03] transition-transform duration-200 cursor-pointer ${isCompleted ? 'border-green-400' : 'border-slate-700'
                  }`}
              >
                <CardHeader
                  floated={false}
                ...
                <CardBody className="px-5 py-4">
                  <Typography className="text-gray-300 text-sm min-h-[48px] line-clamp-2">
                    {description || `Explore ${name} backend resources.`}
                  </Typography>
                  <div className="mt-4">
                    <Progress
                      value={percent}
