...
const BackEndTopics = () => {
  ...
  useEffect(() => {
    ...
    const fetchTopicsAndProgress = async () => {
      try {
        ...
        console.error('Error fetching backend topics:', err);
        setError('Unable to load backend topics. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleTopicClick = (topic) => {
    ...
    navigate(`/courses/backend/${languageId}/${topicSlug}?topicId=${topic._id}`);
  };
